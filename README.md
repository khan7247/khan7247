## 👋 Hi, I’m a Technologist

I build, fix, automate, and occasionally question systems — across **cloud computing, IT infrastructure, automation, web, and security**.  
I work with **Windows & Linux**, set up and manage **Cloud Service Provider (CSP) accounts**, and execute real-world tasks like deployments, access control, and system configuration (yes, including the boring-but-critical stuff).

I also work with **AI & automation**, **web development & SEO**, **digital marketing support**, and **video/photo editing** — because modern tech rarely lives in just one box.  
I believe in **clean systems, clear communication, documented fixes**, and humor as a survival skill.

> _“Everything is fine. Until it’s not. Then we fix it.”_

---

## 🧰 Tech Stack & Skills

| Category | Skills |
|-------|-------|
| 🖥️ Systems | Windows, Linux, OS Configuration, Troubleshooting |
| ☁️ Cloud | GCP, Azure, CSP Account Setup, IAM, Resource Deployment |
| 🔐 Security | Cybersecurity Fundamentals, Threat Awareness, OPSEC |
| 🤖 AI & Automation | AI Tools, Workflow Automation, Generative AI Basics |
| 🌐 Web | Web Development Support, SEO, Performance Optimization |
| 📈 Digital | Digital Marketing Support, Analytics Assistance |
| 🎬 Creative | Video Editing, Photo Editing, Content Optimization |
| 🐍 Programming | Python, C, C++, Java, HTML |

---

## 📜 Certifications & Courses

| Provider | Certification |
|-------|---------------|
| University of Maryland | Cybersecurity Course |
| Google Cloud | Google Cloud Computing Foundations Certificate, Google Cloud Essentials, Introduction to Generative AI |
| IBM | Information Technology Fundamentals, Artificial Intelligence Fundamentals, Cloud Computing Fundamentals |
| Cisco Networking Academy | Operating System Basics, Networking Basics, Python Essentials 1, Python Essentials 2 |
| Great Learning Academy | Virtual Cloud Computing, Microsoft Azure Essentials, Advanced Cybersecurity – Threats & Governance |

---

## 🏷️ Badges

![Linux](https://img.shields.io/badge/Linux-000?logo=linux&logoColor=white)
![Windows](https://img.shields.io/badge/Windows-0078D6?logo=windows&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?logo=googlecloud&logoColor=white)
![Azure](https://img.shields.io/badge/Microsoft_Azure-0089D6?logo=microsoftazure&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![Security](https://img.shields.io/badge/Cybersecurity-222?logo=shield&logoColor=white)
![AI](https://img.shields.io/badge/AI_%26_Automation-FF6F00?logo=openai&logoColor=white)

---

## ⚙️ Current Status

- 🧠 Learning continuously  
- ☁️ Building in the cloud  
- 🔧 Fixing production (calmly)  
- 📚 Documenting what future-me will forget  

